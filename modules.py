...
Original file is located at
    https://colab.research.google.com/drive/1JOLkhZGb1AKY02G6GFhMi8eomuRhkd9C
"""
...
class SoftMax(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        input = np.exp(input) / np.exp(input).sum(axis=1, keepdims=True)
        diag = input[:, None, :] * np.eye(input.shape[1])
        t = np.transpose(input[:, None, :], (0, 2, 1)) @ input[:, None, :]
        self.gradInput = ((diag - t) * gradOutput[:, None, :]).sum(axis=2)
        return self.gradInput
...
class LogSoftMax(Module):
    ...
    def updateOutput(self, input):
        # start with normalization for numerical stability
        self.output = np.subtract(input, input.max(axis=1, keepdims=True))
        self.output = input - np.log(np.exp(input).sum(axis=1, keepdims=True))
        return self.output
    
    def updateGradInput(self, input, gradOutput):
        input = np.subtract(input, input.max(axis=1, keepdims=True))
        input = np.exp(input) / np.exp(input).sum(axis=1, keepdims=True)
        diag = np.ones((input.shape[0], 1, input.shape[1]), dtype=np.float) * np.eye(input.shape[1])
        self.gradInput = np.multiply((np.subtract(diag, input[:, None, :])), gradOutput[:, :, None]).sum(axis=1)
        return self.gradInput
...
class BatchNormalization(Module):
    ...
    def updateOutput(self, input):
        ...
        self.output = (input - batch_mean[None, :]) / (np.sqrt(batch_variance[None, :] + self.EPS))
        return self.output
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        gradient_1 = -(gradOutput * (input - batch_mean) / (batch_variance + self.EPS) ** (3/2)).sum(axis=0)
        gradient_1 = gradient_1 * (input - batch_mean) / input.shape[0]
        
        gradient_2 = -(gradOutput / np.sqrt(batch_variance + self.EPS)).sum(axis=0) / input.shape[0]        
        
        self.gradInput = gradOutput * (1 / np.sqrt(batch_variance + self.EPS)).sum(axis=0) + gradient_1 + gradient_2
...
class Dropout(Module):
    ...
    def updateOutput(self, input):
        if self.training == True:
            self.mask = scipy.stats.bernoulli.rvs(1 - self.p, size=input.shape)
...
class LeakyReLU(Module):
    ...
    def updateOutput(self, input):
        self.output = np.multiply(np.add(np.multiply(input < 0, self.slope), input >= 0), input)
        return  self.output
    
    def updateGradInput(self, input, gradOutput):
        self.gradInput = np.multiply(gradOutput, np.add(np.multiply(input < 0, self.slope), input >= 0))
        return self.gradInput
...
class ELU(Module):
    ...
    def updateOutput(self, input):
        self.output = np.add(np.multiply(input, input > 0), np.multiply(np.multiply(self.alpha, (np.exp(input) - 1)), input <= 0))
        return  self.output
    
    def updateGradInput(self, input, gradOutput):
        f_x = np.add(np.multiply(input, input > 0), np.multiply(np.multiply(self.alpha, (np.exp(input) - 1)),input <= 0))
        second_condition = np.multiply(input < 0, np.add(f_x, self.alpha))
        self.gradInput = np.multiply(gradOutput, np.add(input > 0, second_condition))
        return self.gradInput
...
def adam_optimizer(variables, gradients, config, state):  
    ...
    for current_layer_vars, current_layer_grads in zip(variables, gradients): 
        for current_var, current_grad in zip(current_layer_vars, current_layer_grads):
            ...
            current_var_first_moment = (config['beta1'] * var_first_moment, 
                                        (1 - config['beta1']) * current_grad)
            
            current_var_second_moment = (config['beta2'] * var_second_moment, 
                                         (1 - config['beta2']) * current_grad ** 2)
            
            np.add(
                current_var_first_moment[0], 
                current_var_first_moment[1], 
                out=var_first_moment)
            
            np.add(
                current_var_second_moment[0], 
                current_var_second_moment[1], 
                out=var_second_moment)
            
            current_varience = var_first_moment / (np.sqrt(var_second_moment) + config['epsilon'])
            
            current_var -= np.multiply(lr_t, current_varience)
